...
try:
    ...
    warn('The optional modules of `thd` is not installed. THD Json geometry is not available.')
...
def thd_projection_geometry(api) -> dict:
    ...
    projection_geometry['focal_spot_orientation_quat'] = Rotation.from_matrix(source_orientation).as_quat().tolist()
    projection_geometry['detector_center_position_mm'] = detector_position.tolist()
    projection_geometry['detector_center_orientation_quat'] = Rotation.from_matrix(detector_orientation).as_quat().tolist()
